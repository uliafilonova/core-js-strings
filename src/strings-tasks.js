...
//   const sec = seconds.toString().length < 2 ? `0${seconds}` : seconds;
//   return `${min}:${sec}`;
// }
...
function encodeToRot13(str) {
  ...
  const pot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str
    .split('')
    .map((val) => (alphabet.includes(val) ? pot13[alphabet.indexOf(val)] : val))
    .join('');
}
